...
#if defined(NDEBUG)
#error "Solarium cannot be compiled without assertions."
#endif
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight == 0) {
        nSubsidy = 3000050 * COIN;
    } else if (nHeight == 20){
        nSubsidy = 550000 * COIN;
    } else if (nHeight > 0 && nHeight <= 10000) {
        nSubsidy = 5 * COIN;
    } else if (nHeight > 1000 && nHeight <= 775600) {
        nSubsidy = 20 * COIN;
...
{
    RenameThread("solarium-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // XLRC: We use certain sporks during IBD, so check to see if they are
